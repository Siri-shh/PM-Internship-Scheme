...
import { useAuth } from "@/lib/AuthProvider"; // <- import the auth hook
...
export default function HomePage() {
  ...
  function enterPortal(key: "student" | "company" | "admin", path: string) {
    try {
      const current = sessionStorage.getItem("portalSelected");

      // If user is already authenticated and has a portalSelected:
      // - if it's the same portal: proceed
      // - if it's different: prevent switching and send them back to their portal
      if (isAuthenticated && current) {
        if (current === key) {
          // they tried to re-enter the same portal — allow it
          setLocation(path);
          return;
        } else {
          // logged-in user already tied to another portal — do not allow switching
          // redirect them to the portal they are actually in
          setLocation(`/${current}`);
          return;
        }
      }

      // Normal flow for unauthenticated users (or first-time selection):
      sessionStorage.setItem("portalSelected", key);
    } catch (e) {
      // sessionStorage may be blocked — still navigate
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            ...
            >
              <Card
                onClick={() => enterPortal("student", "/student")}
                className="h-full hover-elevate group cursor-pointer"
            ...
            >
              <Card
                onClick={() => enterPortal("company", "/company")}
                className="h-full hover-elevate group cursor-pointer"
                data-testid="card-company-portal"
            ...
            >
              <Card
                onClick={() => enterPortal("admin", "/admin")}
                className="h-full hover-elevate group cursor-pointer"
