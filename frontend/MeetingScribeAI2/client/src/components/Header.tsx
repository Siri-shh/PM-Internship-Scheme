...
export function Header({ showNav = true }: HeaderProps) {
  ...
  // kept for external usage if needed later
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          {showNav && (
            // Render nav items; if a portal is selected, show only a small label (NO Back button)
            <nav className="hidden md:flex items-center gap-1">
              {portalSelected ? (
                <div className="flex items-center gap-3">
                  {/* Keep the portal label but remove any Back to Home button */}
        ...
        {/* Mobile nav: show portal label only when a portal is selected (NO Back to Home button) */}
